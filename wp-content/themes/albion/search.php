...
if( albion_hide_page_banner() == 1 ) { }else { ?>
	<div class="page-title-area item-bg1"  <?php if(isset($albion_opt['search_bg']['url']) && $albion_opt['search_bg']['url']!='') { ?> style="background-image: url( <?php echo esc_url($albion_opt['search_bg']['url']); ?> );" <?php  } ?>>
		<div class="container">
			<div class="page-title-content <?php echo esc_attr($alignment); ?>">
				<h2><?php printf( esc_html__( 'Search Results For: %s', 'albion' ), '<span>' . get_search_query() . '</span>' ); ?></h2>
			</div>
		</div>
		<div class="lines">
			<div class="line"></div>
