...
<?php
// Breadcrumb Title
if(isset($albion_opt['blog_title'])) {
	$title = $albion_opt['blog_title'];
} else {
	$title = esc_html__('Blog','albion');
}
	...
	<div class="page-title-area item-bg1"  <?php if(isset($albion_opt['blog_bg']['url']) && $albion_opt['blog_bg']['url']!='') { ?> style="background-image: url( <?php echo esc_url($albion_opt['blog_bg']['url']); ?> );" <?php  } ?>>
		<div class="container">
			<div class="page-title-content <?php echo esc_attr($alignment); ?>">
				<h2><?php if(!$title == ''){ echo esc_html($title); }else{ esc_html_e('Blog', 'albion'); } ?></h2>
                <?php
            	  if(albion_breadcumb() != 1){ ?>
                    <ul>
                        <li><a href="<?php echo esc_url( home_url( '/' ) ); ?>"><?php esc_html_e('Home', 'albion')?></a></li>
                        <li><?php if(!$title == ''){ echo esc_html($title); }else{ esc_html_e('Blog', 'albion'); } ?></li>
                    </ul><?php
            	} ?>
			</div>
		</div>
